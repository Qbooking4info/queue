...
**Updated:** September 2026 · Supabase / PostgreSQL · RLS enabled on all tables
...
| `queue_rank_override` | numeric | yes | Manual reorder key (added `20260821000003`) — `renumber_doctor_queue()` orders by `COALESCE(queue_rank_override, extract(epoch from checked_in_at))`, so a manual move (patient self-delay, front-desk Move) survives future renumbers. Set only by `move_appointment_in_queue()`, itself only callable from the service-role `POST /api/appointments/[id]/queue-position` route — never written directly by app code |
| `next_up_alert_sent_at` | timestamptz | yes | One-shot guard (added `20260821000003`) for the "you're next, get ready" push — set the first time a `checked_in` row at `queue_position=2` is alerted (fired from `start_consultation`), so it doesn't repeat on every later queue touch while still at position 2 |
| `check_in_date` | date | yes | |
| `checked_in_at` | timestamptz | yes | Exact moment a row entered `checked_in`/`in_progress`, set once by the `set_checked_in_at` trigger (never overwritten on a later touch, e.g. `assign_doctor`). Queue order is `emergency DESC, COALESCE(queue_rank_override, checked_in_at) ASC` (since `20260821000003`) — this is what actually determines queue position, not arrival order into the code path |
...
`hospital_id NOT NULL AND doctor_user_id NULL` (hospital-mediated — `doctor_id` may still be NULL here,
assigned later at check-in via `PATCH /api/appointments/[id]`'s `assign_doctor` action; the first version
of this constraint wrongly required `doctor_id NOT NULL` too and failed against live rows) or
`hospital_id NULL AND doctor_id NULL AND doctor_user_id NOT NULL` (direct booking). Keeps every existing
...
- `renumber_queue_after_change` (AFTER INSERT OR UPDATE OF `status`, `doctor_id`, `assigned_doctor_id`, `urgency`, `checked_in_at`, `queue_rank_override`) — calls `renumber_doctor_queue()` for every doctor+day a row's change could affect (its current doctor(s), and on UPDATE, whichever it previously belonged to if that changed). Deliberately scoped to those specific columns: `renumber_doctor_queue()`'s own UPDATE only touches `queue_position`/`estimated_wait`, which aren't in that list, so its writes don't re-fire this trigger — omitting that scoping would recurse.

`renumber_doctor_queue(hospital_id, doctor_id, check_in_date)` (function, `SECURITY DEFINER`) — takes `pg_advisory_xact_lock` on `(doctor_id, check_in_date)` first, so two concurrent callers for the same doctor+day serialize instead of both reading the same "0 others ahead" state (this is what previously let two check-ins both land on `queue_position = 1`). Then renumbers *every* `checked_in`/`in_progress` row for that doctor+day via `ROW_NUMBER() OVER (ORDER BY (urgency = 'emergency') DESC, COALESCE(queue_rank_override, extract(epoch from checked_in_at)) ASC NULLS LAST, created_at, id)` (the `queue_rank_override` fallback added `20260821000003`) and recomputes `estimated_wait` from each doctor's own average `consult_duration_secs`. Renumbering the whole set (not just assigning a slot to whoever just joined) also means a departure (complete/cancel/no-show) closes the gap for everyone behind them, not just arrivals getting a fresh count.

`move_appointment_in_queue(p_appointment_id, p_new_position)` (function, `SECURITY DEFINER`, added `20260821000003`, revoked from `authenticated`) — manually repositions a single `checked_in` appointment within its own doctor+day queue, called only from `POST /api/appointments/[id]/queue-position` (staff: any direction; patient: own appointment, later-only, enforced by the route not the function). Takes the same advisory lock as `renumber_doctor_queue` (re-entrant, safe since this function calls that one at the end). Translates the requested *global* position into an index within the appointment's own urgency tier (emergency vs. everything else — the same two-tier split `renumber_doctor_queue`'s `ORDER BY` uses, not a three-way split by the literal `urgency` string), computes a new `queue_rank_override` as the midpoint between whichever two ranks will end up adjacent (fractional/gap ranking, the same technique as Trello/Jira card ordering — avoids rewriting every row on each move), and calls `renumber_doctor_queue()` to refresh integer positions immediately.
...
> `get_hospital_staff_roster(p_hospital_id)` (function, `STABLE SECURITY DEFINER`, `20260727000001`)
> is the read path for this whole section from the Hospital app. RLS on `hospital_admins` /
> `clinic_admins` only lets a caller read their *own* row, so a direct client query returned an
> empty or self-only list silently — the function does its own "caller is an active admin at this
> hospital" check instead and returns `{ staff: [...], doctors: [...] }` in the shape the screen
> renders. Since `20260910000000` the `doctors` array also includes deactivated doctors and each
> carries `is_active`, so the app can show and reactivate them (its one other caller,
> `HospitalScheduleScreen`'s doctor picker, filters to active client-side).
...
| `level` | text | yes | Cadre/rank — e.g. `Consultant`, `Senior Registrar`, `Registrar`, `SMO`, `MO`, `Intern`. Free text (no enum — set matches real-world hospital convention, not this app's to enumerate), self-declared by the doctor. Added `20260820000001`, mirrored on `doctor_profiles` |
...
| `avg_rating` | float8 | yes | Denormalised from reviews |
| `review_count` | integer | yes | |
| `is_active` | boolean | yes | Per-hospital activate/deactivate lever (one `doctors` row per affiliation, so deactivating is scoped to that hospital). A deactivated doctor stops appearing in that hospital's booking and queue flows until reactivated; nothing is deleted. Toggled from web `/dashboard/doctors` and, since `20260910000000`, the Hospital app's Staff screen — both call `PATCH /api/doctors/[id]` |
| `created_at` | timestamptz | yes | |
| `updated_at` | timestamptz | yes | |
...
| `title` | text | yes | e.g. `Dr.` |
| `level` | text | yes | Cadre/rank, same free-text convention as `doctors.level` — see above |
| `specialty_id` | uuid | yes | FK → specialties |
| `bio` | text | yes | |
...
| `20260817000001` | Direct-to-doctor booking: `appointments.hospital_id`/`doctor_id` now nullable, adds `doctor_user_id` + `home_visit_address` + `appointments_booking_shape_check`; fixes `check_duplicate_active_booking()` for the hospital-less shape; adds `doctor_profiles`, `doctor_qualification_documents`, the private `doctor-credentials` Storage bucket, and a `Doctors can read own direct appointments` policy. First attempt at the CHECK constraint required `doctor_id NOT NULL` on every hospital booking and failed against 10 live rows (doctor assigned at check-in, not booking time) — fixed to only require `hospital_id NOT NULL` |
| `20260820000001` | Doctor cadre/level: adds `doctors.level` and `doctor_profiles.level` (free text, self-declared, no enum) |
| `20260821000001` | Short Doctor ID: adds `users.doctor_code` (unique, NOT NULL, 6-char unambiguous-alphabet code via `generate_doctor_code()` + a `BEFORE INSERT` trigger, backfilled for existing rows) — replaces the raw `users.id` UUID as what a hospital admin types into "Link Existing Doctor" |
| `20260821000002` | `get_doctor_queue`: return `hospital_id` alongside each row, needed by the doctors app live queue's new inline Refer quick-action |
| `20260821000003` | Manual queue reordering: adds `appointments.queue_rank_override` (numeric, fractional/gap-ranking key) and `next_up_alert_sent_at` (one-shot alert guard); `renumber_doctor_queue()`'s ORDER BY now keys off `COALESCE(queue_rank_override, extract(epoch from checked_in_at))` instead of raw `checked_in_at`, so a manual reorder survives future renumbers; new `move_appointment_in_queue(p_appointment_id, p_new_position)` SECURITY DEFINER function, revoked from `authenticated` (service-role API route only) |
| `20260821000004` | Fixes `vitals_audit_log`'s `"Front desk can read clinic vitals"` policy to also match `hospital_admins.role = 'front_desk'` (a hospital-wide front-desk account), not just `clinic_admins` — that account type could never read the vitals audit log before this |
...
| `20260828000001` | **Ambulance dispatch supply visibility** — production had 11 transport requests, all `no_unit_available`, 0 dispatch offers ever made: `find_candidate_units` requires a GPS fix newer than 120s, and Android battery optimisation routinely suspends the crew app past that. Loosens/reworks the freshness gate so a unit that pinged recently-ish stays dispatchable, and surfaces the supply funnel (`candidates_found`, filtered/rejected counts) so an empty round is diagnosable |
| `20260828000002` | **Coverage monitor** — the same 2026-08-28 audit found `units_on_shift = 0`: units exist, nobody is signed on. Adds the analysis behind `/dashboard/ambulances/coverage` (and the Hospital app's Coverage screen) that distinguishes a coverage gap (no rota) from an adoption gap (fleet registered, not on duty) from a capacity gap, so an operator sees it before a patient in an emergency does |
| `20260828000003` | **Hospital type vocabulary** — web and mobile were writing disjoint free-text values into `hospitals.type` (web: `hospital`/`clinic`/`specialist_center`/`diagnostic`; mobile: `General`/`Teaching`/`Federal`/…). Nothing branched on it, but the column conflated "what care" with "who owns it". Splits into a defined vocabulary along one axis; both clients aligned |
| `20260828000004` | **Location clock skew** — a crew phone with a wrong system clock stamped `recorded_at` 70 minutes in the past, and dispatch (correctly, by the rule as written) treated the unit as stale and excluded it. Makes the freshness check tolerant of clock skew so a bad device clock no longer removes a real on-duty unit from the network |
| `20260903000001` | **Doctor multi-clinic assignment** — a doctor can be assigned to several clinics within one hospital but is "active" in only one at a time. `doctors.clinic_id` keeps its exact current meaning ("currently active clinic"); every operational read is unchanged. New `doctor_clinics` (`doctor_id`, `clinic_id`) is the assignment *pool* — which clinics a doctor may be active in — written by the clinic-detail "Doctors in this clinic" assign/unassign/Set-Active UI on web and in the Hospital app |
| `20260910000000` | **Roster returns inactive doctors** — `get_hospital_staff_roster`'s `doctors` sub-select drops its `is_active = true` filter and adds `is_active` to each row, so the Hospital app's Staff screen can show a deactivated doctor and offer an Activate button (parity with web `/dashboard/doctors`). Two lines of the function body; auth check, `staff` sub-select, and grants unchanged. No table change — `doctors.is_active` already existed. The RPC's one other caller, `HospitalScheduleScreen`'s doctor picker, filters `is_active !== false` client-side (not `=== true`, so a missing flag counts as active during the app-build-vs-migration window) |
